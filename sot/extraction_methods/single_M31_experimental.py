...
for V,H_perp,R_AHE,R_PHE,Js,H_oe in zip(V_vals,H_perp_vals, R_AHE_vals, R_PHE_vals, Js_vals, H_oe_vals):
    ...
    for htext in HtextAmplitudes:
        ...
        fig, ax = plt.subplots()
        plt.plot(phi_vals, y_vals, label = htext)
        ax.set(xlabel = "field", ylabel = "V2w")
        plt.title(V)
        plt.legend()
        ...
        H_fl_vals[-1].append(H_fl*mu0*1000)
        H_dl_vals[-1].append(H_dl*mu0*1000)
    ...
    HextAmplitudes = np.array(HtextAmplitudes, dtype=int)*0.01/mu0
    invHextDamp = 1/(HextAmplitudes-H_perp)
    fig, ax = plt.subplots()
    plt.plot(invHextDamp, CAs, label = V)
    ax.set(xlabel = "1/hext", ylabel = "Ca")
    plt.title("fit or not fit")
    plt.legend()

plt.close()
fig, axs = plt.subplots(2, gridspec_kw = {'wspace':0, 'hspace':0})
for dl_vals, h in zip(np.transpose(eta_dl_vals), HextAmplitudes*mu0):
    axs[0].plot(V_vals, dl_vals, marker="o", label="dl at Hext "+str(round(h, ndigits=2))+" [T]")
for fl_vals, h in zip(np.transpose(eta_fl_vals), HextAmplitudes*mu0):
    axs[1].plot(V_vals, fl_vals, marker="o", label="fl at Hext "+str(round(h, ndigits=2))+" [T]")
for ax in axs:
    ax.legend()
plt.show()
...
plt.close()
fig, axs = plt.subplots(2, gridspec_kw = {'wspace':0, 'hspace':0})
for dl_vals in zip(np.transpose(eta_dl_vals),HtextAmplitudes):
    axs[0].plot(V_vals, np.array(H_dl_vals), marker="o", label="dl")
for fl_vals in np.transpose(eta_fl_vals):
    axs[1].plot(V_vals, np.array(H_fl_vals), marker="o", label="fl")
plt.show()

print("Hd in mt")
print(np.array(H_dl_vals))
print("Hf in mt")
print(np.array(H_fl_vals))