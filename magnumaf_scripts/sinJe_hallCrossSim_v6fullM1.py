...
# v6fullM1    : Checking from random states
###################
...
Jesystem = 1.97e11 #1.97e11 <-> 5V #*tiny_scale**2 #Jlist[tempindex]*tiny_scale
...
def sweep(vecHextdir, Hmax, steps, relax_time, Jesystem, action, mi=[0.,0.,1.]):
    ...
    for h in listHsweep:
        ...
        for i in np.sin(np.arange(steps_sine+1)/steps_sine*2*np.pi):
            ...
            output_namevti = (filepath, action, h*Constants.mu0*1000, i)
            state.write_vti("%sm_%s_%.2fmT_sin%.2f"% output_namevti)
...
#m0 = cross3d * Magnetization.homogeneous(nx, ny, nz, [0.,0.,1.])
sweep(np.array([1,0,0]), 0.060/Constants.mu0, 11, 5e-9, Jesystem, action='sweep_m+', mi=[0.,0.,1.]) 
#sweep(np.array([1,0,0]), 0.075/Constants.mu0, 7, 4e-9, Jesystem)
#m0 = cross3d * Magnetization.homogeneous(nx, ny, nz, [0.,0.,-1.])
sweep(np.array([1,0,0]), 0.060/Constants.mu0, 11, 5e-9, Jesystem, action='sweep_m-', mi=[0.,0.,-1.])
#relax_m(10e-9)
