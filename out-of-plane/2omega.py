...
def mzlowfield(time, p):
    j = p.currentd * np.sin(2 * 3.1415927 * p.frequency * time)
    prefactorpol = j * p.hbar/(2 * p.e * p.Js * p.d)
    hdl = j * prefactorpol * p.etadamp
    hk = 2 * p.K1/p.Js
    b = p.hext[1] / hk 
    c = hdl / hk
    print(c)
    d = (1 + p.eta**2)*c + 2*b*c + b**2
    den1 = 0
    if (1-d.all()) > 0: den1 = (1-d)**0.5 - 2*c*(c+b)
    #return p.hext / (p.eta * )
    return ( ( den1 - p.eta*((1-d)**0.5)*p.d**2 )/( den1 + ((c+b)**2)/(1-d)**0.5  ) )**0.5
...
savedata(paramters, signal2w, fieldrangeT, "sin_Hx_mzp" + name)
#showplot()
